...
#define IOCTL_DISCONNECT_VIRTUAL_DISPLAY CTL_CODE(FILE_DEVICE_UNKNOWN, 0x804, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define IOCTL_RECONNECT_VIRTUAL_DISPLAY CTL_CODE(FILE_DEVICE_UNKNOWN, 0x805, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
typedef struct _VIRTUAL_DISPLAY_DISCONNECT_PARAMS {
	GUID MonitorGuid;
} VIRTUAL_DISPLAY_DISCONNECT_PARAMS, * PVIRTUAL_DISPLAY_DISCONNECT_PARAMS;

typedef struct _VIRTUAL_DISPLAY_RECONNECT_PARAMS {
	GUID MonitorGuid;
} VIRTUAL_DISPLAY_RECONNECT_PARAMS, * PVIRTUAL_DISPLAY_RECONNECT_PARAMS;

typedef struct _VIRTUAL_DISPLAY_RECONNECT_OUT {
	LUID AdapterLuid;
	UINT TargetId;
} VIRTUAL_DISPLAY_RECONNECT_OUT, * PVIRTUAL_DISPLAY_RECONNECT_OUT;
